  ...
  function renderCoursesPage(data) {
    ...
    function getFilteredItems() {
      ...
      var matched = data
        ...
        .filter(function (x) {
          return (!q || (window.__cheetahTextMatch ? window.__cheetahTextMatch(x.haystack, q) : x.haystack.indexOf(q) >= 0)) && (!categoryValue || x.item.category === categoryValue);
        });
    ...
    searchInput.addEventListener("input", function () {
      if (window.__cheetahLogSearch) window.__cheetahLogSearch(searchInput.value, getFilteredItems().length, "課程總覽");
    });
